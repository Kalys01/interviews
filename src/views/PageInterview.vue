...
<template>
  ...
  <div v-else-if="interview?.id && !isLoading" class="content-interview">
    <app-card>
      <template #title>Собеседование компании {{ interview.company }}</template>
      <template #content>
        ...
        <div class="flex flex-wrap gap-3 mb-3">
          <div class="flex align-items-center">
            <app-radio
              inputId="intervuewResult1"
              name="result"
              value="Refusal"
              v-model="interview.result"
            />
            <label for="intervuewResult1" class="ml-2">Отказ</label>
          </div>
          <div class="flex align-items-center">
            <app-radio
              inputId="intervuewResult2"
              name="result"
              value="Offer"
              v-model="interview.result"
            />
            <label for="intervuewResult2" class="ml-2">Оффер</label>
          </div>
        </div>
